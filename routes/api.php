...
Route::controller(FilmController::class)
    ->group(function () {
        ...
        Route::post('/films', 'store')->middleware('auth:sanctum')->name('films.store'); // todo в рамках задачи на доабвление фильма
        Route::get('/films/{film}', 'show')->name('films.show');
        Route::patch('/films/{film}', 'update')->middleware('auth:sanctum')->name('films.update'); // todo в рамках задачи на доабвление фильма
    });
